...
所有实验在同一硬件环境下运行，使用 sentence-transformers/all-MiniLM-L6-v2 生成 384 维 embeddings，top_k=2 检索最相关 chunk。每种配置在全部 20 个评估查询上运行，收集 Relevance 评分（1-5）、Task Completion（二元）、回答词数和延迟数据。Flan-T5 运行于 CPU 环境，Mistral-7B-Instruct 运行于 GPU 环境（含量化备选）。
